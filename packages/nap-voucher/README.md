...
Every snippet on this page is checked two ways: `test/docsTypecheck.test.ts`
extracts the code blocks **from this file** — and from integration guide §3.5 —
and type-checks them against the real source, so a renamed export or a changed
option breaks the build rather than rotting the docs; and
`test/readmeExample.test.ts` executes the wiring and pins the construction-time
error messages quoted below. A documented
